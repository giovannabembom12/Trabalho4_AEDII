    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            for (int k = 0; k < num_buscas; k++) {
                ...
                ResultadoBFS *r = bfs_executar(g, 0);
                double fim = tempoAtual();
                tempos_busca[k] = fim - inicio;

                if (k == 0) {
                    bfs_imprimir_ordem(r);
                    bfs_imprimir_arvore(r);
                }
                bfs_destruir(r);

                printf("        Busca %2d: %.8f s\n", k + 1, tempos_busca[k]);
            }

            double tempos_media = media(tempos_busca, num_buscas);
            printf("   A média de %d buscas nesse grafo foi: %.8f s\n",
